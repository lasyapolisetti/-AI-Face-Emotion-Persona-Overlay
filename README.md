...
## 🔧 My Contributions (Lasya)

This repository is a fork that I customized and extended. My work includes:

- Customized the neon HUD overlay (window title and glitch text) with my own design and labeling.  
- Implemented an **emotion logging system** that records timestamp, emotion label, confidence, and persona to `emotion_log.csv` for every frame.  
- Added a **Session Emotion Summary** feature that analyzes the log and prints statistics (frequency and percentage of each emotion, and dominant emotion for the session) when the app closes.  
- Set up and ran the full pipeline locally with a Python virtual environment on Windows, resolving dependency and camera issues.  
